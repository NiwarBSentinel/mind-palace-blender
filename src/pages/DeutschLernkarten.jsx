...
export default function DeutschLernkarten() {
  ...
  function openNew() {
    setEditingId(null)
    setForm({ frage: '', antwort: '', mnemonik: '', kategorie: 'Deutsch C1' })
    setShowForm(true)
  }

  function openEdit(card) {
    setEditingId(card.id)
    setForm({ frage: card.frage, antwort: card.antwort, mnemonik: card.mnemonik || '', kategorie: card.kategorie || 'Deutsch C1' })
    setShowForm(true)
  }
  ...
  async function saveCard(e) {
    ...
    const payload = {
      ...
      kategorie: form.kategorie.trim() || 'Deutsch C1',
    }
    if (editingId) {
  ...
  for (const card of filteredCards) {
    const cat = card.kategorie || 'Deutsch C1'
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {!loading && cards.length > 0 && (
        <div className="flex gap-3 mb-6 flex-wrap">
          <button
            onClick={() => navigate('/sprachen/deutsch/lernkarten/practice?mode=due')}
      ...
      {showForm && (
        <form onSubmit={saveCard} className="mb-8 p-6 rounded-xl bg-[#12122a] border border-[#1e1e3a] space-y-4">
          ...
          <input
            type="text"
            placeholder="Kategorie (z.B. Deutsch C1, Deutsch B2)"
            value={form.kategorie}
            onChange={(e) => setForm({ ...form, kategorie: e.target.value })}
            className="w-full px-4 py-2.5 rounded-lg bg-[#0a0a1a] border border-[#2a2a4a] text-slate-200 placeholder-slate-500 focus:outline-none focus:border-green-500 transition"
          />
          <textarea
            placeholder="Mnemonik oder Beispielsatz"
            value={form.mnemonik}
          ...
          <div className="flex gap-2 justify-end">
            <button type="button" onClick={cancelForm} className="px-4 py-2 rounded-lg text-slate-400 hover:bg-[#1e1e3a] transition cursor-pointer">
              Abbrechen
            </button>
            <button type="submit" disabled={!form.frage.trim() || !form.antwort.trim()} className="px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-white font-medium transition cursor-pointer">
              Speichern
            </button>
          </div>
        </form>
      ...
      {!loading && cards.length > 0 && (
        <div className="relative mb-6">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition cursor-pointer text-lg leading-none">×</button>
          )}
        </div>
      )}

      {search && !loading && (
        <p className="text-sm text-slate-400 mb-4">{filteredCards.length} Karten gefunden</p>
      )}

      {loading ? (
        <p className="text-center text-slate-400">Lade Karten...</p>
      ) : cards.length === 0 ? (
        <p className="text-center text-slate-500 py-8">Noch keine Deutsch-Lernkarten. Erstelle eine neue Karte oder speichere Wörter aus dem C1 Vokabeltrainer!</p>
        ...
        <div className="space-y-8">
          {categories.map((cat) => (
            <div key={cat}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-300">{grouped[cat].length}</span>
              </div>
                  ...
                  <div key={card.id} className="p-4 rounded-xl bg-[#12122a] border border-[#1e1e3a] group">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center gap-2">
                        <span className="text-xs px-2 py-0.5 rounded-full bg-[#1e1e3a] text-slate-400">{card.kategorie || 'Deutsch C1'}</span>
                        {(() => {
                          if (!card.repetitions && card.repetitions !== 0) return <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300">Neu</span>
                          if (isDue(card.next_review)) return <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-300">Heute fällig</span>
          ...
          <div className="text-center pt-4">
            <button
              onClick={() => navigate('/sprachen/deutsch/lernkarten/practice')}
